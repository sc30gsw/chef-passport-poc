...
現時点ではローカル実行でデモできます。APIキーなしでも、3人のプリセットシェフはコミット済みキャッシュから動作します。

1. [セットアップ](#セットアップ)の手順でアプリを起動し、`http://localhost:3000` を開く
...
6. `AI_GATEWAY_API_KEY` を設定した場合は、次のライブ導線も試す
...
- Vercelへデプロイし、本番URLとデモ画面のスクリーンショットをREADMEへ追加する
