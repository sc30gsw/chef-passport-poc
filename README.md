...
`AI_GATEWAY_API_KEY` を設定すると、任意の経歴でライブ生成できます。フラグはなく、キーの有無だけで判定します。未設定時はプリセットのキャッシュ再生のみ動作し、自由入力は型付きの拒否レスポンスを返します。同時実行は全体で 1 件までで、衝突も拒否します。さらにプロセス単位のライブ生成予算（トークンバケット。連続 6 件、以後 1 分あたり 1 件）が連打を止めます——同時実行数と違い、回数そのものの上限です。インスタンスをまたぐ上限は AI Gateway 側の 1 キーあたりの上限額だけなので、公開時はそちらの設定が前提になります。
